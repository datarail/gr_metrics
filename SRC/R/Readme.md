...
write.table(assay(drc_output), file = "filename.tsv", quote = FALSE,
sep = "\t", row.names = FALSE)
# Write original data plus GR values to comma-separated file
write.table(metadata(drc_output)[[1]], file = "filename.csv", quote = FALSE,
sep = ",", row.names = FALSE)
```
...
- Hill coefficient of the sigmoidal curve (**Hill**), which reflects how steep 
...
In addition, the scripts report the r-squared of the fit and evaluate the 
significance of the sigmoidal fit based on an F-test. If the fit is not 
significant (p < 0.05, or any arbitrary value), the sigmoidal fit is replaced 
by a constant value (flat fit). The cutoff value for the p-value can be set 
above 1 for bypassing the F-test. Additional information and considerations 
